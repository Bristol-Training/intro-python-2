...
Take your code for encoding from the previous chapter and edit it so that the part that does the conversion is moved into a function called `encode`. The function should take one argument and return the encoded morse string.
...
- Code to decode morse to English is given below:
  ```python
  letter_to_morse = {
      'a':'.-', 'b':'-...', 'c':'-.-.', 'd':'-..', 'e':'.', 'f':'..-.', 
      'g':'--.', 'h':'....', 'i':'..', 'j':'.---', 'k':'-.-', 'l':'.-..', 'm':'--', 
  ...
  ```
- Edit this so that the part that does the conversion (everything from `english = []` to `"".join(english)`) is moved into a function. The function should take one argument, `message` and return the decoded english message. 
